...
public:
Client(std::shared_ptr<Channel> channel, string name){
...
void Chat (string username) {
	...
	thread writer(
		[username, read_write]() {  
			string initial_input = "Set Stream";
...
int main(int argc, char** argv) {
	...
	while(getline(cin, command_line_input)){
		...
		string s = command_exe(connect_to_server, username, command_line_input);
        	...
        	if(s == "CHAT")	break;
		if(s == "NO SUCH COMMAND") cout << "NO SUCH COMMAND" << endl;
	}
